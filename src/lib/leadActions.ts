...
export async function completeAction(opts: {
  ...
  lead: Lead;
  user_id: string;
  /** New stage to set on lead, if action implies a transition */
  newStage?: Stage;
  /** Bump last_contact_at (always for outreach actions) */
  bumpContact?: boolean;
  /** Schedule follow-up via last_contact_at + 3d (only created when contact_at passes) */
  ...
  // Determine activity type and next action
...
export async function markReplied(lead: Lead, user_id: string) {
  ...
  // close any open action and create a reply action
  await supabase
    .from("next_actions")
...
export async function changeStage(lead: Lead, newStage: Stage, user_id: string) {
  ...
  await logActivity(lead.workspace_id, lead.id, user_id, "stage_changed", {
    from: lead.stage,
    to: newStage,
  });
  // If moved to SIGNED or LOST: close open actions
